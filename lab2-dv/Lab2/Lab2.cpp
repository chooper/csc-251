...
int _tmain(int argc, _TCHAR* argv[])
{
   ...
   while (1) {
        ...
        if (eventptr->evtype == FROM_LAYER2 ) 
          delete(eventptr->rtpktptr);        /* free memory for packet, if any */
        delete(eventptr);                    /* free memory for event struct   */
      }
...
void init()                         /* initialize the simulator */
{
  ...
  if (LINKCHANGES==1)   {
   evptr = (struct event *) new struct event;
...
/* to do something with the packet after we return back to him/her */ 
 mypktptr = (struct rtpkt *) new struct rtpkt;
...
/* finally, compute the arrival time of packet at the other end.
 ...
 for (q=evlist; q!=NULL ; q = q->next) 
    if ( (q->evtype==FROM_LAYER2  && q->eventity==evptr->eventity) ) 
      lastime = q->evtime;
 evptr->evtime =  lastime + (float)(2.*jimsrand());
